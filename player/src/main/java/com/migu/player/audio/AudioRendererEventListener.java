    ...
    public void enabled(DecoderCounters decoderCounters) {
      if (handler != null) {
          handler.post(new Runnable() {
              @Override
              public void run() {
                  castNonNull(listener).onAudioEnabled(decoderCounters);
              }
          });
      }
    }
    ...
    public void audioSessionId(int audioSessionId) {
      if (handler != null) {
          handler.post(new Runnable() {
              @Override
              public void run() {
                  castNonNull(listener).onAudioSessionId(audioSessionId);
              }
          });
      }
    }
    ...
    public void skipSilenceEnabledChanged(boolean skipSilenceEnabled) {
      if (handler != null) {
          handler.post(new Runnable() {
              @Override
              public void run() {
                  castNonNull(listener).onSkipSilenceEnabledChanged(skipSilenceEnabled);
              }
          });
      }
    }
