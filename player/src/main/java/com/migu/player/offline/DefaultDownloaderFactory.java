...
//    this(cacheDataSourceFactory, /* executor= */ Runnable::run);
      this(cacheDataSourceFactory, new Executor() {
          @Override
  ...
  private static SparseArray<Constructor<? extends Downloader>> createDownloaderConstructors() {
    ...
    try {
      array.put(
          ...
          getDownloaderConstructor(
              Class.forName("com.migu.player.source.dash.offline.DashDownloader")));
    ...
    try {
      array.put(
          C.TYPE_HLS,
          getDownloaderConstructor(
              Class.forName("com.migu.player.source.hls.offline.HlsDownloader")));
    } catch (ClassNotFoundException e) {
    ...
    try {
      array.put(
          ...
          getDownloaderConstructor(
              Class.forName(
                  "com.migu.player.source.smoothstreaming.offline.SsDownloader")));
    } catch (ClassNotFoundException e) {
